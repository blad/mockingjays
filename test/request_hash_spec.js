...
describe('RequestHash with Empty Body', function () {
  ...
  describe('toString()', function () {
    ...
    it('should not consider query-params in the queryParameterBlacklist', function () {
      const hashWithBlacklist = new RequestHash({ request: exampleWithQueryParams, queryParameterBlacklist: ['secret'] });
      const hashWithBlacklist2 = new RequestHash({ request: exampleWithQueryParams, queryParameterBlacklist: ['public'] });
      const hashWithoutBlacklist  = new RequestHash({ request: exampleWithQueryParams });
      expect(hashWithBlacklist.toString()).to.not.equal(hashWithoutBlacklist.toString());
      expect(hashWithBlacklist.toString()).to.not.equal(hashWithBlacklist2.toString());
      expect(hashWithoutBlacklist.toString()).to.not.equal(hashWithBlacklist2.toString());
    });
  });
